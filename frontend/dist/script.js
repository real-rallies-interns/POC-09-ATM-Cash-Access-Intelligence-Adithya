...
const els = {
    ...
    areaAResult: document.getElementById("area-a-result"),
    areaBResult: document.getElementById("area-b-result"),
    coverageScore: document.getElementById("coverage-score"),
    areaScore: document.getElementById("area-score"),
    underservedArea: document.getElementById("underserved-area"),
    compareResult: document.getElementById("compare-result"),
    comparisonVerdict: document.getElementById("comparison-verdict"),
    travelBurden: document.getElementById("travel-burden"),
    demographicRisk: document.getElementById("demographic-risk"),
    nearestAtm: document.getElementById("nearest-atm"),
    userLocation: document.getElementById("user-location"),
    infraInsight: document.getElementById("infra-insight")
};
function setText(el, value) {
    if (el)
        el.textContent = value;
}
function getPointsByArea(areaName) {
    return accessPoints.filter((p) => p.area === areaName);
}
function getAreaByName(areaName) {
    return areas.find((a) => a.area === areaName);
}
function getCoverageScore(points) {
...
function getTravelBurden(score) {
    if (score >= 70)
        return "5-10 min";
    if (score >= 50)
        return "10-15 min";
...
function populateAreaDropdowns() {
    ...
    areas.forEach((area, index) => {
        ...
        if (index === 0)
            areaAEl.value = area.area;
        if (index === 1)
            areaBEl.value = area.area;
    });
}
function highlightArea(area, color, label) {
...
function renderUnderservedZone(area) {
    L.circle([area.lat, area.lng], {
        radius: 22000,
        color: "#ef4444",
        fillColor: "#ef4444",
...
function compareAreas() {
    ...
    if (scoreDiff <= 10) {
        setText(els.compareResult, `${areaAName} and ${areaBName} show balanced access coverage.`);
        setText(els.comparisonVerdict, "Balanced Comparison");
    }
    else {
        setText(els.compareResult, `${lowerAreaName} has lower access coverage compared to ${higherAreaName}.`);
        setText(els.comparisonVerdict, `Coverage Advantage: ${higherAreaName}`);
    }
    setText(els.travelBurden, getTravelBurden(lowerScore));
    setText(els.demographicRisk, areaDemographics[lowerAreaName]?.risk ?? "Moderate");
    setText(els.infraInsight, `${lowerAreaName} requires stronger ATM/bank distribution to improve regional cash accessibility.`);
...
function updateUserNearestPoint(userLat, userLng) {
    ...
    L.marker([nearestPoint.lat, nearestPoint.lng])
        .addTo(compareLayerGroup)
        .bindPopup(`<b>Nearest Cash Point</b><br>${nearestPoint.name}<br>${nearestPoint.type}`);
}
async function init() {
    try {
        ...
        accessPoints = (await pointsRes.json());
